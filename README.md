...
```
You will see the token in response header for user `ankur`. Note that the status code `401` will be returned if you provide incorrect username or password. And similarly, get token for user `admin`:
```bash
...
| `ankur` token (role `USER`)       |            403 |           200 |            200 |
